...
def create_wishlist(wishlist_data: WishlistCreate, db: Session = Depends(get_db)):
    # Проверка существования пользователя через user_service
    try:
        response = requests.get(f"{USER_SERVICE_URL}/users/{wishlist_data.user_id}")
    ...
    # Генерация идей для подарков на основе интересов
    ...
    # Возврат списка желаний с подарками
    db.refresh(wishlist)
